...
$l_sql = "SELECT idsupplier FROM $table_supplier WHERE mode = 'EURODISPATCH'";
$c_db->query($l_sql);
if (!$c_db->numrows)
{
  print("pas de fournisseurs de type EURODISPATCH");
  return 0;
}
...
if ($option == "mail") mail($adm->mail,"EURODISPATCH",$l_text,"From: $g_sitename <$g_kernixemail>\n", "-f$g_kernixemail");
