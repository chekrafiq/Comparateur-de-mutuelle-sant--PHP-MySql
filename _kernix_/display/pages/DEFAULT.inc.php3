...
?>
          <tr onmouseover='javascript:this.bgColor="<?=$l_color1?>"' onmouseout='javascript:this.bgColor="<?=$l_color2?>"'>
           <td height="20" style="padding:5px 0">
...
<?php } ?>
        ...
        <td valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="15" class="contenu">
          <tr>
           <td valign="top"><br>
	    <h1 class="titre_<?=$ref->template?>" style="margin:0;padding:0"><?=$l_title?></h1>
	    <br>
<?php if ($l_subtitle) { ?>
	    <h3 class="sous_titre" style="margin:0;padding:0"><?=$l_subtitle?></h3>
	    <br>
<?php } ?>
<?php if ($ref->accroche) { ?>
...
<?php
if ($p_devisaction) include("$g_modulespath/devis/sub/index.inc.php3");
else include("$g_designpath/common/modules.inc.php3");
?>
