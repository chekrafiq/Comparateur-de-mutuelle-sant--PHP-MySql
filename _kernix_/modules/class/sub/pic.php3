...
class pic
{
  ...
  function draw_pic ($tab)
    { 
      ...
      // creation de l'image en memoire  !!! - 30 AJOUTER PAR FX PR SUPPRIMER ESPACE A DROITE 
      $image = imagecreate($this->pic_width - 30, $this->pic_height);
  ...
  function get_xy ($tab) //recherche la valeur max en ordonnees et le nombre de val en abscisses
    {
      $i = 0;
      $this->nb_x = 0;
      $this->max_y = 0;
  ...
  function set_size ($tab) // affiche les infos de l'axe des abcisses
	...
	$text_len = ImageTTFBBox($this->fonth, -90, $this->path, $tab[$a - 1][0]);
	
	//cherche le texte le plus long depassant de l'image en hauteur
	if ($ytext_pos + $text_len[5] - $this->pic_height > $max_height)
	  $max_height = $ytext_pos + $text_len[5] - $this->pic_height;
	  
	//cherche le texte le plus long depassant de l'image en largeur
	$xtext_pos = $this->x($a) - ($this->xscale / 2);
	if ($xtext_pos + $text_len[5] - $this->pic_width > $max_width)
	  $max_width = $xtext_pos + $text_len[5] - $this->pic_width;
	...
	//	  $do_resize = 1;
	$old_height = $this->pic_height;
	$this->pic_height += $max_height - 20;
	$this->y_translation += $max_height - 20;
      }
...
//le nom des valeurs
      ImageTTFText ($image, $this->fonth, 0, 3, 10, $grey, $this->path, $this->yname);
      
      //l'origine 
      ImageTTFText ($image, $this->fonth, 0, 0, $this->y(0) + 10, $black, $this->path, $this->origin);
      
      //affiche le nom de l'abscisse
//	     ImageTTFText ($image, $this->fonth, 0, $this->pic_width - 25, $this->pic_height - 45, $grey, $this->path, $this->xname);
      
      //affiche le titre de l'image au milieu
      $title_len = ImageTTFBBox($this->fonth, 0, $this->path, $this->maintitle);
      $pos = ($this->pic_width / 2) - (($title_len[0] + $title_len[2]) / 2); 
      ImageTTFText ($image, $this->fonth, 0, $pos, 10, $title_color, $this->path, $this->maintitle);
	...
	if ($this->y($i) + 3 < $this->y(0) - 5)
	  ImageTTFText ($image, $this->fonth, 0, 0, $this->y($i) + 3, $black, $this->path, $old_i);
	$i += $this->max_y / $nb_draw;
      }
	...
	ImageTTFText ($image, $this->fonth, 0, 0, $this->y($i/2) + 3, $black, $this->path, $i/2);
      }
	...
	//si append a ete activé, on ajoute la valeur au texte de la legende de chaque baton de l'histogramme
	if ($this->append == 1 && isset($tab[$i - 1][0]))
	  $tab[$i - 1][0] .= " (" . $tab[$i - 1][1] . ")";
	
	//affiche le texte coorespondant
	ImageTTFText ($image, $this->fonth, -45, $this->x($i) - ($this->xscale / 2) - 3, $ytext_pos, $black, $this->path, $tab[$i - 1][0]);
      }
