...
$l_sql = "SELECT * FROM $table_result  WHERE idform = '$p_idform' ORDER BY date";
$c_db->query($l_sql);
if ($c_db->numrows == 0)
{
...
while ($i < $tmp)
{
     ...
     print("<a href=\"$g_urlroot/$g_modulespath/form/adm/index.php3?p_formaction=viewresult&p_idformresult=" . $c_db->result($i,"idformresult") . "\" class=truelink><img src=/pictures/poll/msg.gif border=0></a></td>");
     print("</tr>");
     $i++;
}
