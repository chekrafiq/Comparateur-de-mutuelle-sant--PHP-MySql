...
$l_sql = "SELECT count(idpub) as som, idpub FROM $table_publog WHERE idpub > 0 AND date_format(date,'%Y') = '$p_year' AND date_format(date,'%m') = date_format('$l_date','%m') GROUP BY idpub ORDER BY som DESC";
$c_db->query($l_sql);
...
print("<tr><td class=color2 align=center colspan=2> &#187; Top click &#171; </td></tr>");
$i=0;
