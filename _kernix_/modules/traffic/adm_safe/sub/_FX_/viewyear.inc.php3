...
//$l_sql = "SELECT idvisitor FROM $table_visitor WHERE date_format(firstvis,'%Y') = '$p_year' AND cookie = '1'";
//$c_db->query($l_sql);
...
$l_sql = "SELECT idvisitor FROM $table_visitor WHERE flash = '1' AND date_format(firstvis,'%Y') = '$p_year' AND cookie = '1'";
$c_db->query($l_sql);
...
$l_sql = "SELECT idvisitor FROM $table_visitor WHERE nbrvis > 1 AND cookie = '1' AND date_format(firstvis,'%Y') = '$p_year'";
$c_db->query($l_sql);
$l_nboften = $c_db->numrows;
$l_oftenrate = ($l_nboften / $l_nbvisitor) * 100;
$l_oftenrate = sprintf("%.2f ",$l_oftenrate);

$l_sql = "SELECT SUM(TO_DAYS(lastvis) - TO_DAYS(firstvis)) AS sum FROM $table_visitor WHERE date_format(firstvis,'%Y') = '$p_year' AND cookie = '1' AND lastvis != firstvis";
$c_db->query($l_sql);
$l_freq = 0;
if ($c_db->numrows && $l_nboften)
$l_freq = $c_db->result(0,"sum") / $l_nboften;
...
<table width=98% align=left border=0>
 ...
 <tr>
  <td class=color2 width=75% align=right>nombre de visites
  </td>
 ...
 <tr>
  <td class=color2 align=right>pourcentage de visiteurs passant par un autre site
  </td>
  <td class=color3 align=left>&nbsp;<?php printf("%.2f",$l_refrate);print(" %"); ?>
  </td>
 </tr>
 <tr>
  <td class=color2 align=right>pourcentage de visiteurs venant d'un mail
  </td>
...
$l_sql = "SELECT idvisitor FROM $table_visitor WHERE date_format(firstvis,'%m') = date_format('$l_date','%m') AND cookie = '1' AND date_format(firstvis,'%Y') = '$p_year'";
$c_db->query($l_sql);
