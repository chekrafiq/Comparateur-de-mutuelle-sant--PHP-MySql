...
$l_sql = "SELECT idvisitor FROM $table_visitor WHERE date_format(firstvis,'%m') = '$p_nummonth' AND  date_format(firstvis,'%Y') = '$p_year' AND cookie = '1'";
$c_db->query($l_sql);
$l_nbvisitorthismonth = $c_db->numrows;

$l_sql = "SELECT COUNT(idlog) AS dt FROM $table_log WHERE newvis = 1 AND date_format(date,'%m') = '$p_nummonth' AND date_format(date,'%Y') = '$p_year'";
$c_db->query($l_sql);
$l_nbvisthismonth = $c_db->result(0,"dt");
...
$l_sql = "SELECT count(idlog) AS dt FROM $table_log WHERE newvis = '1' AND NOT date_format(date,'%w') IN (0,6) AND date_format(date,'%m') = '$p_nummonth' AND date_format(date,'%Y') = '$p_year'";
$c_db->query($l_sql);
$l_weekendrate = 100 - (($c_db->result(0,"dt") / $l_nbvisthismonth) * 100);
...
$l_visitnbpages = round($l_total / $i);
