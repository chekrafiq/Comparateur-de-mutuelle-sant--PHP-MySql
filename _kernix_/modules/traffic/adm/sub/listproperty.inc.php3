...
$l_sql = "SELECT L.idproperty AS idproperty, count(L.idproperty) as n, P.propertyname AS name FROM $table_log AS L, $table_property AS P WHERE L.idproperty != '0' AND L.idproperty != '' AND L.idproperty = P.idproperty AND L.date LIKE '$p_year-$p_nummonth-%' GROUP BY L.idproperty ORDER BY n DESC";
$c_db->query($l_sql);
...
print("<tr><td class=color2 align=center colspan=2> &#187; catégories &#171; </td></tr>");
for ($i=0;$i<$l_nb;$i++)
{
