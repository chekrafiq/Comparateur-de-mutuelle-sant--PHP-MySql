...
// nb visit
$l_sql = "SELECT count(*) FROM $table_log WHERE newvis = '1' AND date LIKE '$p_year-%'";
$c_db->query($l_sql);
$l_nbvisit = $c_db->result(0,0);

// nb visitor
$l_sql = "SELECT count(*) FROM $table_visitor WHERE firstvis LIKE '$p_year-%'";
$c_db->query($l_sql);
$l_nbvisitor = $c_db->result(0,0);

// nb page
$l_sql = "SELECT count(*) FROM $table_log WHERE date_format(date,'%Y') = '$p_year'";
$c_db->query($l_sql);
$l_nbpage = $c_db->result(0,0);

// % des visitor venu plus d'une fois
$l_sql = "SELECT count(*) FROM $table_visitor WHERE nbrvis > 1 AND firstvis LIKE '$p_year-%'";
$c_db->query($l_sql);
...
$l_sql = "SELECT count(*) FROM $table_visitor WHERE remotereferer != '' AND firstvis LIKE '$p_year-%'";
$c_db->query($l_sql);
...
$l_sql = "SELECT count(*) FROM $table_visitor WHERE (urlfromfirstvis = '::MAIL::' OR urlfromlastvis = '::MAIL::') AND firstvis LIKE '$p_year-%'";
$c_db->query($l_sql);
...
<table width="100%" align="left" border="0">
 ...
 <tr>
  <td class="color2" align="right">pourcentage de visiteurs étant venus plus d'une fois</td>
...
<!--  <td class="color3" align="left">&nbsp;<?=$l_oftenrate?> %-->
  </td>
 </tr>
 <!--
 <tr>
  <td class="color2" align="right">fréquence des venues (jours)</td>
...
// nb visit ce mois
$l_sql = "SELECT count(*) FROM $table_log WHERE newvis = 1 AND date LIKE '$p_year-$l_month-%'";
$c_db->query($l_sql);
$l_nbvisthismonth = $c_db->result(0,0);

// nb visitor
$l_sql = "SELECT count(*) FROM $table_visitor WHERE firstvis LIKE '$p_year-$l_month-%'";
$c_db->query($l_sql);
...
$l_sql = "SELECT DISTINCT date_format(date,'%m') AS nummonth, date_format(date,'%b') AS namemonth FROM $table_log WHERE date LIKE '$p_year-%' AND newvis = 1 ORDER by nummonth DESC";
$c_db->query($l_sql);
$n = $c_db->numrows;
...
$l_sql = "SELECT count(*) as n, screen FROM $table_visitor WHERE firstvis LIKE '$p_year-%' AND screen != '' GROUP BY screen ORDER BY n DESC LIMIT 0,5";
$c_db->query($l_sql);
$n = $c_db->numrows;
for ($i=0;$i<$n;$i++)
{
...
if ($l_nbvisitor) {
	$vl_tab = array();
	$l_sql = "SELECT browser, count(*) as nb FROM $table_visitor WHERE firstvis LIKE '$p_year-%' GROUP BY browser";
	$c_db->query($l_sql);
	while($obj = $c_db->object_result()) {
		switch($obj->browser) {
			case 'IE60':
				$l_MSIE6 = ($obj->nb/$l_nbvisitor) * 100;
				break;
			case 'IE55':
				$l_MSIE55 = ($obj->nb/$l_nbvisitor) * 100;
				break;
			case 'IE50':
				$l_MSIE5 = ($obj->nb/$l_nbvisitor) * 100;
				break;
			case 'IE40':
				$l_MSIE4 = ($obj->nb/$l_nbvisitor) * 100;
				break;
			case 'NS60':
				$l_Nets6 = ($obj->nb/$l_nbvisitor) * 100;
				break;
			case 'NS4+':
				$l_Nets4p = ($obj->nb/$l_nbvisitor) * 100;
				break;
			case 'NS40':
				$l_Nets4 = ($obj->nb/$l_nbvisitor) * 100;
				break;
			default:
				break;
		}
	}
	$l_other = 100 - $l_MSIE6 - $l_MSIE55 - $l_MSIE5 - $l_MSIE4 - $l_Nets6 - $l_Nets4p - $l_Nets4;
}
?> 
   <table width="100%" cellspacing="2" cellpadding="0">
    <tr><td align="right" class="list" width="48%">IE 6</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_MSIE6); ?> %</td></tr>
    <tr><td align="right" class="list">IE 5.5</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_MSIE55); ?> %</td></tr>
    <tr><td align="right" class="list">IE 5</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_MSIE5); ?> %</td></tr>
    <tr><td align="right" class="list">IE 4</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_MSIE4); ?> %</td></tr>
    <tr><td align="right" class="list">Netscape 6</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_Nets6); ?> %</td></tr>
    <tr><td align="right" class="list">Netscape 4+</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_Nets4p); ?> %</td></tr>
    <tr><td align="right" class="list">Netscape 4</td><td class="list"> &nbsp;<?php printf("%.2f ",$l_Nets4); ?> %</td></tr>
...
if ($l_nbvisitor)
{
  $l_sql = "SELECT count(idvisitor) as n  FROM $table_visitor WHERE os = 'MAC' AND firstvis LIKE '$p_year-%'";
  $c_db->query($l_sql);
  if ($c_db->numrows != 0) $l_MAC = ($c_db->result(0,"n") / $l_nbvisitor) * 100;
  else $l_MAC = 0;
  
  $l_sql = "SELECT count(idvisitor) as n  FROM $table_visitor WHERE os = 'UNIX' AND firstvis LIKE '$p_year-%'";
  $c_db->query($l_sql);
