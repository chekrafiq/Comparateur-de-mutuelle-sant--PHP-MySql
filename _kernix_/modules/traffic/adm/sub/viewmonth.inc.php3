...
// nb visitor
$l_sql = "SELECT count(*) FROM $table_visitor WHERE firstvis LIKE '$p_year-$p_nummonth-%'";
$c_db->query($l_sql);
$l_nbvisitorthismonth = $c_db->result(0,0);

// nb visit ce mois
$l_sql = "SELECT count(*) FROM $table_log WHERE newvis = 1 AND date LIKE '$p_year-$p_nummonth-%'";
$c_db->query($l_sql);
$l_nbvisthismonth = $c_db->result(0,0);

// % de visite le weekend
$l_sql = "SELECT count(*) FROM $table_log WHERE newvis = 1 AND NOT(date_format(date,'%w') IN (0,6)) AND date LIKE '$p_year-$p_nummonth-%'";
$c_db->query($l_sql);
$l_weekendrate = 100 - ($c_db->result(0,0) / $l_nbvisthismonth * 100);

// duree moy d'une visiste
$l_sql = "SELECT min(date) AS min, max(date) AS max FROM $table_log WHERE idsession != '' AND date LIKE '$p_year-$p_nummonth-%' GROUP BY idsession";
$c_db->query($l_sql);
$i = 0;
$l_total = 0;
while ($obj = $c_db->object_result())
{
  $l_total += 30 + strtotime($obj->max) - strtotime($obj->min);
  $i++;
}
$l_visitduration = $l_total / $i;

// nb moyen de pages vues
$l_sql = "SELECT count(*) AS n FROM $table_log WHERE idsession != '' AND date LIKE '$p_year-$p_nummonth-%' GROUP BY idsession";
$c_db->query($l_sql);
$i = 0;
$l_total = 0;
while ($obj = $c_db->object_result())
...
// nb de HNI
$l_sql = "SELECT count(*) FROM $table_logaltern WHERE date LIKE '$p_year-$p_nummonth-%'";
$c_db->query($l_sql);
...
<table width="95%">
 <tr>
   <td align="left" class="color1" colspan="2">
    :: ce mois &nbsp;&nbsp;&nbsp; 
    <small><a href=<?php print("$PHP_SELF?p_trafficaction=viewmonth&p_year=$p_year&p_nummonth=$p_nummonth"); ?> class=whitelink><?php print($p_nummonth); ?></a> / </small>
    <a href=<?php print("$PHP_SELF?p_trafficaction=viewyear&p_year=$p_year"); ?> class=whitelink><?php print("$p_year"); ?></a>
   </td> 
 </tr>
 <tr>
 ...
 <tr>
  <td class="color2" align="right">HNI &nbsp;</td>
...
<img src="/extern/getgraph.php3?p_title=visites+par+jour&p_x=550&p_y=150&p_code=traffic/adm/sub/graph_visitbyday.inc.php3&p_ordtitle=visites&p_nummonth=<?=$p_nummonth?>&p_year=<?=$p_year?>">
