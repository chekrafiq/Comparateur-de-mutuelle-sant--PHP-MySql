...
<title>SwissLife Retraite : Devis </title>
...
function validateFormOnSubmit(theForm) {
		...
		reason += validateEmail(theForm.EMAIL);
  		...
  		reason += validateMONTANT(theForm.MONTANT);
...
function validatePhone(fld) {
 ...
 function validateMONTANT(fld) {
    var error = "";
 
    if (fld.value.length == 0) {
        error = "10) Vous n'avez pas entré le Montant souhaité  !.\n";
    
    } else if (fld.value == "Montant souhaité ( minimum 75€/mois )") {
        error = "10) Vous n'avez pas entré le Montant souhaité  !.\n";     
     } return error;  
}
...
<div class="container_10 wrapper m_t_10">
	...
	<div id="content" class="grid_5">
		<h1>SwissLife Retraite</h1>
				<h2>Devis de votre Complémentaire santé.</h2>
		<div class="message_box_wrap notice">Si vous souhaitez obtenir plus d'informations, nous vous prions 
		de bien vouloir<strong style="color:red"> remplir ce formulaire</strong> afin que nous puissions vous établir un devis gratuit et sans engagement : </div>
<form method="post" action="../../../../control/devis_retraite.php" onsubmit="return validateFormOnSubmit(this)">
	...
	<input id="MONTANT" name="MONTANT" class="box w_250" type="text" value="Montant souhaité ( minimum 75€/mois )" onblur="if(this.value=='') this.value='Montant souhaité ( minimum 75€/mois )'" onfocus="if(this.value =='Montant souhaité ( minimum 75€/mois )' ) this.value=''" /> &nbsp;
