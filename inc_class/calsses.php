...
class Client
{
	...
	function getTarif($company_id, $gamme_id, $formule_id, $sexe, $nbr_enfant, $letarif, $tarifEnf, $regime_id, $age, $age_conj)
	{
		...
		//9  ==> augmentation
		if ( $row ) {
		// echo 'Age_prospect: <span style="color:red" >'.$age.' </span>Ans<br>';
		// echo 'Age_Conjoint: <span style="color:red" >'.$age_conj.' </span>Ans<br>';
		 //echo 'ID_remis: <span style="color:green" >'.$row[0].' </span><br>';
        // echo 'remis: <span style="color:green" >'.$row[7].' </span><br>';
		// echo 'Frais Dossier: <span style="color:blue" >'.$row[10].' </span><b>&euro;</b><br>';
		 //echo 'taux: '.$row[8].'<br>';
		 //echo 'gratuite_enfant: '.$row[13].'<br>';
		 //echo 'remis_2_eme_enfant: <span style="color:blue" >'.$row[14].' </span><b>&euro;</b><br>';
		 // echo $sql.'<br>';
		 ...
		 if ($gamme_id==17 && $nbr_enfant > 1 && $sexe=="seul") return 0;
		 if ($gamme_id==17 && $nbr_enfant > 0 && $sexe=="couple") return 0;
		 if ($gamme_id==18 && $nbr_enfant < 2 && $sexe=="seul") return 0;
		 if ($gamme_id==18 && $nbr_enfant < 1 && $sexe=="couple") return 0;		 
			
			// NEW EQUI ($gamme_id==17 && $nbr_enfant >= 2 && !$clt->couple=="couple") || ($gamme_id==17 && $nbr_enfant >= 1 && !$clt->couple=="seul")	
			// methode calcule pour NEOLIAN ENFANT (Gratuit� du 3�me enfant, payant � partir du 4�me
			
			if (($gamme_id==16 && $nbr_enfant >= 4) || ($gamme_id==15 && $nbr_enfant >= 4)) { $letarif = $letarif + ($nbr_enfant >= 4 ?  $tarifEnf * ($nbr_enfant - 1) :  $tarifEnf * $row[13]);}   else { $letarif = $letarif + ($nbr_enfant > $row[13] ?  $tarifEnf * $row[13] :  $tarifEnf * $nbr_enfant);}
			...
			//if (($gamme_id==17 && $nbr_enfant >= 2 && $sexe=="seul")) {$letarif = $letarif * 0.908;} else {$letarif;}
			//if (($gamme_id==17 && $nbr_enfant >= 2 && $clt->couple==null)) {$letarif = $letarif * 0.9;} else {$letarif;}
